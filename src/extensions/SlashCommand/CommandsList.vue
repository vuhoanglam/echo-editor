...
<template>
  <div
    class="bg-white rounded-lg dark:bg-black shadow-xs border border-neutral-200 dark:border-neutral-800 text-black max-h-[min(80vh,24rem)] overflow-auto flex-wrap mb-8 p-1"
    ref="scrollContainer"
  >
    <div class="grid grid-cols-1 gap-0.5 min-w-48" v-if="items?.length">
      <template v-for="(group, groupIndex) in items" :key="group.title">
        <div
          className="text-neutral-500 text-[0.65rem] col-span-full mx-2 mt-2 font-semibold tracking-wider select-none uppercase first:mt-0.5"
        >
          {{ group.title }}
        </div>
        <button
